...
export const Statistics = () => {
  ...
  // 🔥 SOLO ESTO SE AGREGA (lógica dinámica)
  const getMin = () => {
    if (selectedParameter === ParametroTipo.TEMPERATURA) {
      return user.celsius
        ? acuarioSeleccionado.temp_min
        : celsiusToFarenheit(acuarioSeleccionado.temp_min);
    }

    if (selectedParameter === ParametroTipo.PH) {
      return acuarioSeleccionado.ph_min;
    }

    return chartSettings[selectedParameter].min;
  };
  ...
  const getMax = () => {
    ...
    if (selectedParameter === ParametroTipo.PH) {
      return acuarioSeleccionado.ph_max;
    }

    return chartSettings[selectedParameter].max;
  };
